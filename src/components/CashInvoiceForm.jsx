...
 const [invoiceIdState, setInvoiceId] = useState(invoiceId || null);
...
const saveToFirebase = async () => {
  try {
    ...
    if (invoiceIdState) {
      await updateDoc(doc(db, "cashinvoices", invoiceIdState), finalData);
      alert("تم تحديث الفاتورة النقدية!");
    } else {
      const docRef = await addDoc(collection(db, "cashinvoices"), finalData);
      setInvoiceId(docRef.id); // حفظ المعرف الجديد
